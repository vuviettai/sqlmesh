"""Explode Beneficiaries.attachments into sqlmesh_work.btxh_stg_beneficiary_attachments."""
...
from .btxh_helper import explode_beneficiary_attachments
...
TIMESTAMP_COLUMNS = ("_airbyte_extracted_at", "updated_at", "created_at", "attachment_updated_at")
INTEGER_COLUMNS = ("_airbyte_generation_id", "file_id")
...
@model(
    "sqlmesh_work.btxh_stg_beneficiary_attachments",
    description=(
        "Exploded BTXH beneficiary attachments from public.\"Beneficiaries\" "
        "with one row per attachment."
    ),
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            while True:
                ...
                for row in rows:
                    records.extend(explode_beneficiary_attachments(dict(row)))
