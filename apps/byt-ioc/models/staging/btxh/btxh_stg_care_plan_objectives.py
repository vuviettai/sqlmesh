"""Explode CareActivities.carePlan.objectives into sqlmesh_work.btxh_stg_care_plan_objectives."""
...
from .btxh_helper import explode_care_plan_objectives
...
DATE_COLUMNS = ("plan_date", "approval_date", "review_date")
...
@model(
    "sqlmesh_work.btxh_stg_care_plan_objectives",
    description=(
        "Exploded BTXH care plan objectives from public.\"CareActivities\" "
        "with one row per care plan objective."
    ),
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            while True:
                ...
                for row in rows:
                    records.extend(explode_care_plan_objectives(dict(row)))
