"""Explode SocialWorkers.educationHistories into sqlmesh_work.btxh_stg_sw_edu_hist."""
...
from .btxh_helper import explode_social_worker_education_histories
...
@model(
    "sqlmesh_work.btxh_stg_sw_edu_hist",
    description=(
        "Exploded BTXH social worker education histories from public.\"SocialWorkers\" "
        "with one row per education history item."
    ),
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            while True:
                ...
                for row in rows:
                    records.extend(explode_social_worker_education_histories(dict(row)))
